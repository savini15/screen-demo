<template>
  <div
    class="bg-cover bg-center h-screen text-white p-2 flex overflow-hidden"
    v-if="data"
  ...
  >
    ...
    <div class="w-1/2 mr-2 flex flex-col">
      ...
      <!--      地图可视化-->
      <MapChart
        class="opacity-50 bg-slate-800 p-3 mt-2 flex-1"
        :data="data.mapData"
      />
    </div>
  ...
  <div
    v-else
    :style="{ backgroundImage: `url( ${currentBg} )` }"
    class="bg-cover bg-center h-screen text-white p-2 flex overflow-hidden text-6xl justify-center items-center flex-row"
  >
